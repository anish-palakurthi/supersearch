...
def get_chrome_data():
    ...
    chrome_user_data_path = os.path.join(home_dir, "Library", "Application Support", "Google", "Chrome", "Default")
    if not os.path.exists(chrome_user_data_path):
        print(f"Chrome user data directory not found at {chrome_user_data_path}")
        return

    # Path to the History and Bookmarks files
    history_path = os.path.join(chrome_user_data_path, "History")
    bookmarks_path = os.path.join(chrome_user_data_path, "Bookmarks")
    ...
    temp_history_path = "/tmp/chrome_history.sqlite"
